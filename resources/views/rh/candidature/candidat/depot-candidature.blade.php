...
<head>
    ...
    <style>
        ...
        .file-drop-zone {
            border: 2px dashed #0d6efd; /* Primary color */
            cursor: pointer;
            transition: border-color 0.3s ease, background-color 0.3s ease;
            background-color: #e9f5ff; /* Light primary background */
        }
        .file-drop-zone.drag-over {
            border-color: #0a58ca; /* Darker primary */
            background-color: #cce5ff; /* Lighter primary */
        }
...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary shadow-sm">
    <div class="container">
        <a href="javascript:history.back()" class="navbar-brand fw-bold">
            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center mb-3">
                        ...
                        <div>
                            <h2 class="mb-1 fw-bold">{{$offre->titre ?? 'Titre du poste'}}</h2>
                            <div class="d-flex gap-2">
                                <span class="badge bg-success">{{$offre->type_contrat ?? 'Non spécifié'}}</span>
                                <span class="badge bg-light text-dark">{{$offre->salaire ?? 'Non spécifié'}} {{ $offre->devise ?? '' }}/{{ $offre->periode_salaire ?? '' }}</span>
                                <span class="badge bg-light text-dark">{{$offre->experience_requise ?? 'Non spécifié'}}</span>
                            </div>
                        </div>
                    </div>
